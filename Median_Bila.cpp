...
////////////////////////////////////////////////////////////////////
//Problem 3 : (a)
//This code is using Median Filter and Bilateral Filter to remove noise in the image
/////////////////////////////////////////////////////////////////////
#include <stdio.h>
#include <stdlib.h>
#include <iostream>
...
int  main()
{
	...
	int OTB_Image[Size][Size][BytesPerPixel];                           //the temporal image data of output image(int)
 ...
 /////////////////Enlarge the image after Median Filter//////////////
   ...
   /////////////////Bilateral Filter///////////////////////////////
	...
	double sigmaD=5.0;       
	double sigmaS=50.0;
	int *windowB=new int[(2*R+1)*(2*R+1)];                   //store the gray value of all pixel in filter windowG
	for (w=0;w<(2*R+1)*(2*R+1);w++)                             //initial windowG
		windowB[w]=0;
	double *weightD=new double [(2*R+1)*(2*R+1)];   //store the weight in filter window(spacial distance)
	double *weightS=new double [(2*R+1)*(2*R+1)];   //store the weight in filter window(photometric distance)
	double *weight=new double [(2*R+1)*(2*R+1)];      //store the weight in filter window
	for (w=0;w<(2*R+1)*(2*R+1);w++)                            //initial weightD 
		weightD[w]=0;
	for (w=0;w<(2*R+1)*(2*R+1);w++)                            //initial weightS
		weightS[w]=0;
	for (w=0;w<(2*R+1)*(2*R+1);w++)                            //initial weight
		weight[w]=0;
	w=0;
	double delta=0, x=0, y=0;
	double sumW=0;                                  //the sum of weight
    ...
    for (pI=R;pI<Size+R-1;pI++)
		for (qI=R;qI<Size+R-1;qI++)
			for (c=0;c<BytesPerPixel;c++)
			{
				w=0;sumW=0.0;sumC=0.0;
				//put the gray value in windowB[] and covolute with weight
			    for (i=-R;i<R+1;i++)
				    for (j=-R;j<R+1;j++)
					{
						windowB[w]=IT2_Image[pI+i][qI+j][c];
						weightS[w]=(-0.5)*(IT2_Image[pI+i][qI+j][c]-IT2_Image[pI][qI][c])*(IT2_Image[pI+i][qI+j][c]-IT2_Image[pI][qI][c])/(sigmaS*sigmaS);
						w++;
					}
				for (w=0;w<(2*R+1)*(2*R+1);w++)
				{
					weight[w]=exp(weightD[w]+weightS[w]);
					sumW+=weight[w];
				}
				//normalization of the weight	  
				for (w=0;w<(2*R+1)*(2*R+1);w++)         //initial weight
					weight[w]=weight[w]/sumW;

				for (w=0;w<(2*R+1)*(2*R+1);w++)         //initial weight
					sumC+=windowB[w]*weight[w];

				//replace the central gray value in the filter window with the median
				OTB_Image[pI-R][qI-R][c]=sumC;
			}

  ///////////////////Write image/////////////////////
	ofstream tfile("smoothB_image.raw",ios_base::out | ios_base::binary);
	if (!tfile)
	{
	...
	for (pO=0;pO<Size;pO++)
		for (qO=0;qO<Size;qO++)
			for (c=0;c<BytesPerPixel;c++)
			{
				COut_Image[pO][qO][c]=OTB_Image[pO][qO][c]+0x00;    //Convert to hex or bin
